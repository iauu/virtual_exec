...
fn test_op_add_functionality() {
    ...
    let result_int = err_op_add(lhs_int.clone(), rhs_int.clone(), &alloc)
        .expect("op_add for Int+Int should succeed");
    assert_eq!(result_int.as_int().unwrap(), 42);
    println!("Int + Int works as expected.");

    let result_float = err_op_add(lhs_float.clone(), rhs_float.clone(), &alloc)
        .expect("op_add for Float+Float should succeed");
    // Use a small epsilon for float comparison
    assert!((result_float.as_float().unwrap() - 3.75).abs() < f64::EPSILON);
    println!("Float + Float works as expected.");

    let result_unsupported = err_op_add(lhs_int.clone(), lhs_float.clone(), &alloc)
        .expect("op_add for Int+Float should succeed");
